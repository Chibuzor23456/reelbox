...
export default function Series() {
  ...
  useEffect(() => {
    listVod({ type: 'series' })
      ...
      .catch(() => setLoadError('Could not load series.'))
  }, [])

  return (
    <div className="page-gutter py-6">
      <h1 className="mb-4 text-2xl font-semibold text-text">Series</h1>
      ...
      {!loadError && items.length === 0 && (
        <p className="text-sm text-muted">No series yet — the catalogue is still being ingested.</p>
      )}
    </div>
  )
}
